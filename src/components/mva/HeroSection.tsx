...
/* Positions for satellite nodes around center (cx=250, cy=250) */
const satellites = [
...
const NetworkVisual = () => (
  <svg viewBox="0 0 500 500" className="w-full h-full" fill="none">
    ...
    {satellites.map((s, i) => (
      <motion.line key={`l-${i}`} x1="250" y1="250" x2={s.x} y2={s.y} stroke="hsl(253 100% 62% / 0.12)" strokeWidth="1" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: s.delay, duration: 0.6 }} />
    ))}
    {satellites.map((s, i) => (
      <motion.circle key={`n-${i}`} cx={s.x} cy={s.y} r={s.r} stroke="hsl(253 100% 62% / 0.4)" strokeWidth="1.5" fill="hsl(253 100% 62% / 0.08)" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: s.delay + 0.2, duration: 0.4, type: "spring" }} />
    ))}
    <motion.circle cx="250" cy="250" r="48" fill="hsl(253 100% 62%)" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4, duration: 0.5, type: "spring" }} />
    <motion.text x="250" y="244" textAnchor="middle" fill="white" fontSize="14" fontWeight="700" fontFamily="Rajdhani, sans-serif" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}>MVA</motion.text>
    <motion.text x="250" y="262" textAnchor="middle" fill="hsl(253 100% 90%)" fontSize="10" fontWeight="600" fontFamily="Inter, sans-serif" letterSpacing="2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>CORE</motion.text>
  </svg>
);
      ...
      <div className="container mx-auto max-w-[1200px] relative z-10">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
          ...
          <div>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25 }} className="text-base md:text-lg text-muted-foreground max-w-[480px] leading-relaxed mb-6">
          ...
          <motion.div
            ...
            className="relative max-w-[460px] mx-auto hidden md:block"
          >
            <NetworkVisual />
