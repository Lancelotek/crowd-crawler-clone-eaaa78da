        ...
        <div className="container mx-auto max-w-[1200px]">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-14">
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight mb-4">
              {t("blog", "title")}<br /><span className="text-primary">{t("blog", "titleAccent")}</span>
            </h1>
          </motion.div>
          ...
          {loading ? (
            ...
            <p className="text-center text-muted-foreground text-lg">{t("blog", "noPosts")}</p>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map((post, i) => (
                <motion.div key={post.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05, duration: 0.4 }}>
                  <Link to={`${langPrefix}/blog/${post.slug}`} className="group block rounded-card border border-border bg-card overflow-hidden hover:border-primary/30 hover:shadow-lg transition-all">
                    {post.cover_image && (
                      <div className="aspect-[16/10] overflow-hidden">
                        <img src={post.cover_image} alt={buildBlogImageAlt(post, lang as "en" | "pl").alt} width={1200} height={675} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
                      </div>
                    )}
                    <div className="p-5">
                      <div className="flex items-center gap-3 mb-3">
                        {post.category && <span className="text-xs font-semibold text-primary">{post.category}</span>}
                        {post.read_time && <span className="text-xs text-muted-foreground">{post.read_time}</span>}
                      </div>
                      <h2 className="font-display text-lg font-bold leading-snug mb-2 group-hover:text-primary transition-colors">{post.title}</h2>
                      {post.author && <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{post.author}</p>}
                    </div>
                  </Link>
                </motion.div>
              ))}
